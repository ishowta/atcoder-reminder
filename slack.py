...
class Slack():
	...
	def __init__(self, token: str, name: str, icon: str, legacy_token: str = None) -> None:
		self.token = token
		self.name = name
		self.icon = icon
	...
	def postImage(self, name: str, channel: str, title: str, image_url: str = None, image: Any = None) -> None:
	...
	def setReminder(self, channel: str, date: datetime, comment: str) -> None:
		if self.legacy_token is None:
			raise Exception("Legacy token does not set! If you send a slack message by shell, you must set `legacy_token` at construction.")
